...
const FOLLOWUP_MIN_1 = 45;
const FOLLOWUP_MIN_2 = 240;
const FOLLOWUP_MIN_3 = 1440;
const FOLLOWUP_POLL_MS = 60 * 1000;
...
function extractNameFromText(text) {
 ...
 if (/(sim|ok|beleza|claro|show|tanto faz|nao|não|dor|sono|ansiedade|fibromialgia|insônia|insonia)/.test(low) && t.split(" ").length <= 2) {
 ...
 const preferred = cleaned.match(/(?:pode me chamar de|me chama de|me chame de)\s+(.+)$/i);
 const declared = cleaned.match(/(?:me chamo|sou|nome e|nome é)\s+(.+)$/i);
 let candidate = (preferred?.[1] || declared?.[1] || cleaned).trim();
 candidate = candidate.replace(/^(de|do|da)\s+/i, "").trim();

 const stop = /^(pode|chamar|me|de|oi|ola|olá|bom|boa|tarde|dia|noite)$/i;
 let parts = candidate.split(" ").filter(Boolean).filter((p) => !stop.test(p));
...
function asksPlanIncludes(text) {
 const t = norm(text);
 return /(inclui|inclui exatamente|o que vem|o que inclui|consulta agora e depois um retorno|teria consulta agora e depois um retorno|como funciona esse acompanhamento|esse acompanhamento)/.test(t);
}

function asksPrescriptionFlow(text) {
 const t = norm(text);
 return /(saio com a receita|sair com receita|receita e orientacao|receita e orientação|como conseguir o medicamento|prescricao|prescrição)/.test(t);
}

function asksMedicationCost(text) {
 const t = norm(text);
 return /(medicamento.*(caro|custa|valor|preco|preço)|quanto costuma custar por mes|quanto costuma custar por mês|custo mensal|valor mensal|por mes|por mês)/.test(t);
}

function asksScamTrust(text) {
 const t = norm(text);
 return /(golpe|enganacao|enganação|fachada|curso|suplemento|isca|propaganda online|isso e confiavel|isso é confiável|posso confiar)/.test(t);
}

function asksEligibility(text) {
 const t = norm(text);
 return /(sou caso|faz sentido no meu caso|mesmo sem diagnostico|mesmo sem diagnóstico|so para casos graves|só para casos graves|sem diagnostico fechado|sem diagnóstico fechado)/.test(t);
}

function asksDifference(text) {
 const t = norm(text);
 return /(qual a diferenca|qual a diferença|o que diferencia|diferencial|por que com o dr|por que com o doutor|por que escolher|porque escolher|comparando com outros medicos|comparando com outros médicos)/.test(t);
}

function directPriceOpener(text) {
 const t = norm(text);
 return /^(quanto custa|qual o valor|quais os valores|preco|preço|valor)/.test(t);
}
...
No *${plan.label}* você faz a consulta inicial com o Dr. Alef e já fica com um retorno em torno de 30 dias.

Esse retorno serve para revisar como você ficou, ajustar a conduta se necessário e acompanhar o início do tratamento com mais segurança.`;
 }
 if (plan.key === "basic") {
  return `Claro 🙂

Na *${plan.label}* você faz a primeira consulta completa, com cerca de ${plan.subtitle}, para entender seu caso com profundidade e decidir com segurança se esse tratamento faz sentido para você.`;
 }
 return `Claro 🙂

A *${plan.label}* é voltada para quem já passou pela avaliação e precisa revisar evolução, ajustar conduta ou alinhar próximos passos.`;
}

function prescriptionReply() {
 return `Sim 🙂

Se o Dr. Alef entender que faz sentido para o seu caso, ele já orienta os próximos passos com segurança e emite a prescrição quando houver indicação.`;
}

function medicationCostReply() {
 return `Essa é uma dúvida muito importante 🙂

O valor pode variar conforme o tipo de produto e a dose, mas em muitos casos fica em uma faixa de algumas centenas de reais por mês.

O Dr. Alef costuma orientar de forma realista, pensando no que faz sentido para o caso e também no que é viável para o paciente.`;
}

function scamTrustReply() {
 return `É totalmente válido ter esse cuidado 🙂

Aqui o atendimento é uma *consulta médica real*, individualizada, com o Dr. Alef, e não curso, suplemento ou promessa milagrosa.

A ideia é avaliar seu caso com seriedade e dizer com honestidade se esse caminho faz sentido para você ou não.`;
}

function eligibilityReply() {
 return `Não necessariamente 🙂

Muita gente procura a avaliação por sintomas como dor, ansiedade ou dificuldade para dormir, mesmo sem um diagnóstico fechado.

A consulta serve justamente para entender se existe indicação com segurança no seu caso.`;
}

function differenceReply() {
 return `Boa pergunta 🙂

O principal diferencial aqui é que a consulta é médica, individualizada e focada em entender seu caso de verdade — não é atendimento genérico.

Além disso, o Dr. Alef avalia histórico, medicações em uso, segurança, custo-benefício e próximos passos com mais profundidade.

Se você quiser, eu também posso te mostrar como funciona a avaliação ou os horários disponíveis.`;
}

function shouldFastTrackAfterObjection(flags) {
 return !!(flags.asksIfWorks || flags.asksLegal || flags.asksChapado || flags.asksScam || flags.asksDifference || flags.asksEligibility);
}

function shouldPausePaymentForQuestion(text) {
 return asksPlanIncludes(text) || asksPrescriptionFlow(text) || asksMedicationCost(text) || asksLegal(text) || asksChapado(text) || asksScamTrust(text) || asksEligibility(text) || asksDifference(text);
}

function asksLegal(text) {
 const t = norm(text);
 return /(legal no brasil|e legal|é legal|precisa de receita|pode comprar|comprar sem receita|anvisa)/.test(t);
}
...
function buildFollowupReply(state) {
 const nome = state?.nome || "";
 const first = nome ? `${nome}, ` : "";
 if (!state?.problem_text && !state?.focus) {
  return `${first}passando só para não te deixar sem resposta 🙂

Se ainda fizer sentido, me diz em 1 frase o que mais tem te incomodado hoje e eu sigo daqui com você.`;
 }
 if (state?.stage === "WAIT_PAYMENT" && state?.payment?.link) {
  return `${first}vi que sua reserva ficou pausada 🙂

Se ainda fizer sentido seguir, seu link continua disponível por aqui:
${state.payment.link}

Se ficou alguma dúvida antes de confirmar, pode me falar sem problema.`;
 }
 if (state?.date_key && !state?.slot_time) {
  return `${first}vi que nossa conversa ficou pausada 🙂

Se quiser, eu posso retomar daqui e te mostrar os melhores horários disponíveis.`;
 }
 if (state?.slot_time && !state?.selected_plan_key) {
  return `${first}se ainda fizer sentido, seu horário continua encaminhado por aqui 🙂

Se quiser, eu retomo exatamente da parte do plano para ficar mais fácil.`;
 }
 return `${first}vi que nossa conversa ficou pausada 🙂
...
async function processFollowups() {
 try {
  const { rows } = await pool.query("SELECT phone, state FROM wa_users");
  const now = Date.now();
  for (const row of rows) {
   const phone = row.phone;
   const state = row.state || {};
   if (!state?.last_bot_from) continue;
   if (state?.payment?.status === "approved") continue;
   const outboundAt = Number(state.last_outbound_at || state.last_sent_at || 0);
   const inboundAt = Number(state.last_inbound_at || 0);
   const stage = Number(state.followup_stage || 0);
   if (!outboundAt || inboundAt >= outboundAt) continue;

   const mins = (now - outboundAt) / 60000;
   let shouldSend = false;
   if (stage === 0 && mins >= FOLLOWUP_MIN_1) shouldSend = true;
   if (stage === 1 && mins >= FOLLOWUP_MIN_2) shouldSend = true;
   if (stage === 2 && mins >= FOLLOWUP_MIN_3) shouldSend = true;
   if (!shouldSend) continue;

   const body = buildFollowupReply(state);
   try {
    ...
    state.followup_stage = stage + 1;
    state.last_bot_reply = body;
    state.last_outbound_at = Date.now();
    state.last_sent_at = state.last_outbound_at;
...
setInterval(processFollowups, FOLLOWUP_POLL_MS);
...
- trabalhar em um modelo 60% liberdade guiada e 40% regras fixas
...
- Em WhatsApp, seja mais curta do que institucional.
- Não faça anamnese longa demais.
- Se o paciente perguntar algo direto, responda objetivamente em até 2 blocos curtos.
...
function initializeState(state, bot) {
 ...
 state.last_inbound_at = Number(state.last_inbound_at || 0);
 state.last_outbound_at = Number(state.last_outbound_at || 0);
 state.followup_stage = Number(state.followup_stage || 0);
...
app.post("/whatsapp", async (req, res) => {
 ...
 state.last_inbound_at = Date.now();
 state.followup_stage = 0;
 ...
 // 1.5) perguntas de alta prioridade antes do fluxo
 else if (flags.asksScam) {
 reply = scamTrustReply() + (state.nome ? `

Se quiser, eu também posso te explicar como funciona a avaliação.` : `

Se fizer sentido, depois eu sigo daqui com você e te explico como funciona a avaliação.`);
 }
 else if (flags.asksDifference) {
 reply = differenceReply();
 }
 else if (flags.asksEligibility) {
 reply = eligibilityReply() + `

Se quiser, eu posso te mostrar como funciona a avaliação ou os horários disponíveis.`;
 }
 else if (flags.asksPlanDetails) {
 reply = planDetailsReply(state.selected_plan_key || "full");
 }
...
Se quiser, eu também posso te explicar como funciona a primeira consulta.`;
 }
 else if (flags.directPrice || (flags.wantsPrice && state.price_ask_count >= 1)) {
 state.price_ask_count += 1;
 reply = state.price_ask_count >= 2 || flags.directPrice ? `${prePriceValueReply()}

${priceReply()}` : prePriceValueReply();
 if (state.price_ask_count >= 2 || flags.directPrice) state.stage = state.nome ? "ASK_PLAN" : state.stage;
 }
 ...
 state.last_outbound_at = state.last_sent_at;
