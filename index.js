 ...
 *
 * OBJETIVO:
 * - mais humana no início (60% IA / 40% regras)
 ...
 * MODEL_CHAT (opcional, padrão gpt-5.4)
...
if (!PUBLIC_BASE_URL) console.warn("?? PUBLIC_BASE_URL não definido.");

const openai = new OpenAI({ apiKey: OPENAI_API_KEY });
const twilioClient = twilio(TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN);

const CHAT_MODEL = MODEL_CHAT || "gpt-5.4";
...
function extractNameFromText(text) {
    ...
    const directPatterns = [
        /(?:pode me chamar de|me chama de|me chame de)\s+([a-zà-ÿ' -]{2,})$/i,
        /(?:meu nome e|meu nome é|nome e|nome é|me chamo|sou)\s+([a-zà-ÿ' -]{2,})$/i,
    ];
    for (const rx of directPatterns) {
        const m = t.match(rx);
        if (m?.[1]) {
            const candidate = m[1].trim().replace(/^o\s+/i, "");
            const parts = candidate.split(/\s+/).filter(Boolean).slice(0, 4);
            if (parts.length) return parts.map((part) => part.charAt(0).toUpperCase() + part.slice(1).toLowerCase()).join(" ");
        }
    }

    if (/(sim|ok|beleza|claro|show|tanto faz|nao|no|dor|sono|ansiedade|fibromialgia|insnia|insonia)/.test(low) && t.split(" ").length <= 2) {
        if (/(dor|sono|ansiedade|fibromialgia|insnia|insonia)/.test(low)) return null;
    }
    ...
    const stopwords = new Set(["pode", "chamar", "de", "me", "nome", "e", "eh", "sou", "prazer", "oi", "ola"]);
    const parts = candidate.split(" ").filter(Boolean).filter((part) => !stopwords.has(norm(part)));

    if (parts.length < 1 || parts.length > 4) return null;
    if (/^\d+$/.test(candidate)) return null;
    if (/(dor|sono|ansiedade|fibromialgia|artrose|artrite|enxaqueca|coluna|insnia|insonia)/i.test(candidate) && parts.length <= 2) return null;
...
function extractPlanChoice(text) {
    ...
    if (/^(1|opcao 1|opo 1|plano 1)$/.test(t)) return "full";
    if (/^(2|opcao 2|opo 2|plano 2)$/.test(t)) return "basic";
    if (/^(3|opcao 3|opo 3|plano 3)$/.test(t)) return "retorno";

    if (/\b(acompanhamento|consulta com retorno|com retorno|pacote|retorno em 30|acompanhamento medico|acompanhamento mdico|consulta \+ retorno|consulta e retorno|quero a de 447|447)\b/.test(t)) return "full";
    if (/\b(avaliacao especializada|avaliao especializada|avaliacao|avaliao|so a consulta|s a consulta|opcao 2|opo 2|quero a de 347|347|primeira avaliacao|primeira avaliao|consulta inicial|avaliacao inicial|faz mais sentido pra mim agora|acho que a 2|acho que opcao 2|acho que opo 2|prefiro a avaliacao)\b/.test(t)) return "basic";
    if (/\b(retorno avulso|consulta de ajuste|apenas retorno|quero a de 200|200)\b/.test(t)) return "retorno";
...
function planDetailsReply(planKey = "full") {
    if (planKey === "full") {
        return "Claro 🙂\n\nNesse acompanhamento de R$447 voce faz a consulta inicial com o Dr. Alef e ja fica com um retorno em torno de 30 dias.\nEsse retorno serve para revisar como voce ficou, ajustar a conduta se necessario e acompanhar o inicio do tratamento com mais seguranca.";
    }
    if (planKey === "basic") {
        return "A avaliacao de R$347 e a consulta inicial de 45 minutos para entender seu caso com profundidade e ver se esse tratamento faz sentido para voce.\nDepois, se houver necessidade de acompanhamento, isso e decidido com mais calma.";
    }
    return "A consulta de ajuste de R$200 e um retorno avulso, mais indicada para quem ja passou por avaliacao e precisa revisar conduta ou fazer ajuste.";
}

function prescriptionReply() {
    return "Sim 🙂\n\nSe o Dr. Alef entender que faz sentido para o seu caso, ele orienta os proximos passos com seguranca e emite a prescricao quando houver indicacao clinica.";
}

function medicationCostReply() {
    return "Essa e uma duvida muito importante 🙂\n\nO valor pode variar conforme o tipo de produto e a dose, mas em muitos casos fica em uma faixa de algumas centenas de reais por mes.\nO Dr. Alef costuma orientar de forma realista, pensando no que faz sentido para o caso e tambem no que e viavel para o paciente.";
}

function differenceDoctorReply() {
    return "A diferenca da consulta com o Dr. Alef e que ela e individualizada, cuidadosa e sem protocolo generico.\nEle revisa seu historico, o que voce ja tentou, medicacoes em uso e ajusta a conduta com foco em seguranca e realidade do seu caso.";
}

function realConsultReply() {
    return "Sim 🙂\n\nE consulta medica mesmo, feita com avaliacao individual, criterio clinico e orientacao profissional.\nNao e esquema nem algo solto: o objetivo e entender seu caso com seguranca e definir se esse caminho faz sentido para voce.";
}

function noClosedDiagnosisReply() {
    return "Isso nao impede a avaliacao 🙂\n\nMesmo sem um diagnostico totalmente fechado, o Dr. Alef pode revisar seu historico e entender se esse tratamento faz sentido para o seu caso.";
}

function driverSafetyReply() {
    return "Essa e uma preocupacao muito valida 🙂\n\nIsso pode acontecer dependendo da formula e da dose, principalmente quando existe THC.\nQuando a rotina envolve dirigir, isso entra como ponto central da avaliacao para buscar mais seguranca e menor risco de sedacao.";
}

function objectionAfterPaymentReply(state, flags) {
    const extras = [];
    if (flags.asksPlanDetails) extras.push(planDetailsReply(state.selected_plan_key || "full"));
    if (flags.asksPrescription) extras.push(prescriptionReply());
    if (flags.asksMedicationCost) extras.push(medicationCostReply());
    if (flags.asksDifferenceDoctor) extras.push(differenceDoctorReply());
    if (flags.asksRealConsult) extras.push(realConsultReply());
    if (flags.asksNoClosedDiagnosis) extras.push(noClosedDiagnosisReply());
    if (flags.asksDriverSafety || flags.asksChapado) extras.push(driverSafetyReply());
    if (flags.asksLegal) extras.push(legalReply());
    if (!extras.length && flags.saysExpensive) extras.push(buildExpensiveReply());
    if (!extras.length && (flags.saysWillSee || flags.saysUnsure || flags.expressesFear)) extras.push(buildThinkingReply(state));
    if (!extras.length) return null;
    return extras.join("\n\n") + `\n\nSe quiser seguir, seu link continua aqui:\n${state.payment.link}`;
}

async function buildOpeningReply(incomingText, state, flags) {
    const intro = "Oi 🙂\nEu sou a Lia, da equipe do Dr. Alef Kotula. Muito prazer.\nQual e o seu primeiro nome?";
    let answer = "";
    if (flags.asksLegal) answer = legalReply();
    else if (flags.asksChapado) answer = chapadoReply();
    else if (flags.asksDriverSafety) answer = driverSafetyReply();
    else if (flags.asksHowConsultWorks || flags.asksIfOnline) answer = consultationExplanationReply();
    else if (flags.asksPriceDirect) answer = "Claro 🙂\n\nPosso te passar sim os valores, e ja te explico rapidinho a diferenca entre as opcoes para voce entender qual faz mais sentido.";
    else if (flags.asksIfWorks) answer = buildWorksReply(state, incomingText);
    else if (flags.asksDifferenceDoctor) answer = differenceDoctorReply();
    else if (flags.asksPrescription) answer = prescriptionReply();
    else if (flags.asksMedicationCost) answer = medicationCostReply();
    else if (flags.asksNoClosedDiagnosis) answer = noClosedDiagnosisReply();
    else if (flags.asksRealConsult) answer = realConsultReply();
    else if (flags.expressesFear) answer = "Entendo sua inseguranca 🙂\n\nA ideia da consulta e justamente avaliar com criterio se esse tratamento faz sentido para voce, sem promessas irreais e com bastante seguranca.";
    else if (state.problem_text || detectCondition(incomingText)) answer = "Entendi seu ponto 🙂\n\nEu vou te ajudar a entender isso com calma e de forma objetiva.";
    return answer ? `${intro}\n\n${answer}` : intro;
}
...
function detectIntent(text) {
    const t = norm(text);
    const wantsPrice = /\b(preco|preo|valor|quanto custa|investimento|custa|valores)\b/.test(t);
    const asksPriceDirect = wantsPrice;
    const intentPay = /\b(como (pagar|faco para pagar)|pagar|pagamento|pix|cartao|carto|credito|crdito|debito|dbito|boleto|link|parcel|parcela|quero pagar)\b/.test(t);
    const wantsBook = /\b(quero marcar|quero agendar|agendar|marcar|confirmar consulta|quero consulta|gostaria de agendar|tem horario|tem horrio|agenda)\b/.test(t);
    const asksHours = /\b(horarios|horrio|horario|que horas|vagas|disponibilidade)\b/.test(t);
    const confirms = /\b(sim|ok|beleza|claro|confirmo|fechado|vamos|pode ser|serve|confirmar)\b/.test(t);
    const refuses = /\b(nao quero|no quero|pare|para|chega|rude|grosso|nao gostei|no gostei)\b/.test(t);
    const asksStartNow = /\b(como tomar|dose|dosagem|quantas gotas|comecar agora|comear agora)\b/.test(t);
    const urgency = /\b(dor no peito|falta de ar|desmaio|avc|convuls|paralisia|confusao|confuso)\b/.test(t);
    const asksWho = /\b(quem e|quem eh|quem e o dr|quem e o doutor|quem atende|quem  o dr|quem  o doutor)\b/.test(t);
    ...
    const saysIndecisive = /\b(tanto faz|qual voce acha melhor|qual voc acha melhor)\b/.test(t);
    const saysExpensive = /\b(caro|carissima|carssimo|achei caro|muito caro|pesado)\b/.test(t);
    const saysUnsure = /\b(nao tenho certeza|no tenho certeza|nao sei|no sei|sera|ser|to na duvida|t na dvida|duvida|dvida|nunca fiz esse tipo de tratamento|tenho medo|fico insegur)\b/.test(t);
    const asksHowConsultWorks = /\b(como funciona a consulta|como funciona essa consulta|como funciona essa avaliacao|como funciona a avaliao|como funciona a avaliacao)\b/.test(t);
    const asksIfOnline = /\b(e online| online|online mesmo|presencial|precisa ir|tem que ir|ir em algum lugar|precisa ir na clinica|precisa ir na clnica)\b/.test(t);
    const asksLegal = /\b(legal no brasil|e legal| legal|precisa de receita|receita|pode comprar|comprar sem receita|anvisa|regularizado|regularizada|legalizado)\b/.test(t);
    const asksChapado = /\b(chapado|chapar|maconha mesmo|isso e maconha|isso maconha|droga)\b/.test(t);
    const asksPlanDetails = /\b(o que inclui|inclui exatamente o que|inclui o que|esse acompanhamento|consulta agora e depois um retorno|retorno seria para ajustar|diferenca entre os planos|diferena entre os planos|o que esta incluso|o que est incluso|o que tem no plano)\b/.test(t);
    const asksPrescription = /\b(saio com receita|ja saio com a receita|prescricao|prescrio|orientacoes|orientaes|como conseguir o medicamento)\b/.test(t);
    const asksMedicationCost = /\b(medicamento costuma ser muito caro|quanto custa por mes|quanto custa por ms|custo do medicamento|custo mensal|medicamento depois)\b/.test(t);
    const asksDifferenceDoctor = /\b(qual a diferenca da consulta do doutor|qual a diferenca da consulta do dr|diferencial do doutor|diferencial do dr|qual a diferenca)\b/.test(t);
    const asksRealConsult = /\b(consulta medica mesmo|consulta mdica mesmo|esquema|golpe|isso e consulta mesmo|isso consulta mesmo|verdadeiro)\b/.test(t);
    const asksDriverSafety = /\b(dirigir|dirijo|reflexo|alterado|sonolento|sono dirigindo|motorista)\b/.test(t);
    const asksNoClosedDiagnosis = /\b(sem diagnostico fechado|sem diagnstico fechado|ainda nao tenho diagnostico|ainda no tenho diagnstico|sem diagnstico)\b/.test(t);
    const expressesFear = /\b(tenho medo|fico insegur|receio|preocupad|frustrar de novo)\b/.test(t);
    const focus =
        (/\b(insonia|insnia|dormir|sono|acordar)\b/.test(t) && "insonia") ||
        (/\b(ansiedade|panico|pnico|crise)\b/.test(t) && "ansiedade") ||
        (/\b(fibromialgia)\b/.test(t) && "fibromialgia") ||
        (/\b(neuropat|irradia|formigamento|choque|ardencia|ardncia)\b/.test(t) && "dor_neuropatica") ||
    ...
    return {
        ...
        asksPrescription,
        asksMedicationCost,
        asksDifferenceDoctor,
        asksRealConsult,
        asksDriverSafety,
        asksNoClosedDiagnosis,
        expressesFear,
        focus,
    };
}
...
app.post("/whatsapp", async (req, res) => {
    ...
    (async () => {
        try {
            ...
            // 2) abertura com answer-first obrigatorio
            else if (!state.stage && !state.nome) {
                state.stage = "ASK_NAME";
                state.opening_answered = true;
                reply = await buildOpeningReply(finalText, state, flags);
            }
            ...
            // 8) intenção de pagar / pagamento pendente
                ...
                const objectionReply = objectionAfterPaymentReply(state, flags);
                if (objectionReply) {
            ...
            // 16) plano
                ...
                if (flags.asksPlanDetails) {
                    reply = planDetailsReply(state.selected_plan_key || planKey || "full");
                }
                else if (flags.asksPrescription) {
                    reply = prescriptionReply();
                }
                else if (flags.asksMedicationCost) {
                    reply = medicationCostReply();
                }
                else if (flags.asksDifferenceDoctor) {
                    reply = differenceDoctorReply();
                }
                else if (flags.asksRealConsult) {
                    reply = realConsultReply();
                }
                else if (flags.asksNoClosedDiagnosis) {
                    reply = noClosedDiagnosisReply();
                }
                else if (flags.asksDriverSafety || flags.asksChapado) {
                    reply = driverSafetyReply();
                }
                else if (flags.saysExpensive) reply = buildExpensiveReply();
                else if (flags.saysWillSee) reply = buildThinkingReply(state);
                else if (flags.saysUnsure || flags.expressesFear) reply = buildUnsureReply(state, finalText);
                else if (planKey) {
            ...
            state.last_stage_touched_at = Date.now();
